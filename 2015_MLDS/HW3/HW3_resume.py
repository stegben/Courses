...
s = 240 # split training set
sen = 50 # sentence length
epochs = 1
...
for epoch in range(epochs):
	...
	for split in range(s):
		...
		model.fit(train_vec , label_vec , batch_size = 1 , nb_epoch = 1 , validation_split=0.0 , shuffle=False , show_accuracy=True)
...
for key in sorted(test):
	...
	result = model.predict(np.array(i , dtype = theano.config.floatX) , batch_size = 1 , verbose = 0)
