...
model.add(MaxoutDense(2048,512,nb_feature=8))
model.add(Dropout(0.4))

model.add(Dense(512,2048,activation='linear'))
model.add(PReLU(2048))
model.add(Dropout(0.4))
...
trainer = Adadelta(lr = 4.5 , rho = 0.97 , epsilon = 1e-8 )
...
try:
  for i in range(epoch):
    model.fit(X,Y , batch_size = 256,nb_epoch=1,shuffle=True,validation_split=0.0,show_accuracy=False)
    ...
    for k in range(y_pred.shape[0]):
      ans = Y_cv[k,:].tolist().index(1)
      if np.argmax(y_pred[k,:]) == ans:
        correct.append(y_pred[k,ans])
        correct_label.append(ans)
      else:
        wrong.append(np.amax(y_pred[k,:]))
        wrong_label.append(ans)
    # plt.scatter(correct,correct_label)
    # plt.scatter(wrong,wrong_label,color='red',marker='+')
    # plt.show()
    print(np.mean(np.array(correct)))
    print(np.std(np.array(correct)))
    print(np.mean(np.array(wrong)))
    print(np.std(np.array(wrong)))

    if i%8 == 7:
      Y = np.zeros((tr_row_num,48) , dtype = theano.config.floatX)
      lab = model.predict_classes(X)
