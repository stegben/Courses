...
repeat_num = [(77,128,5),(90,100,5),(101,100,5),(151,70,5)]
...
X = scaler.fit_transform(np.concatenate((X,X_cv),axis=0))
...
prob_te_1 = getResultByDiffSeq( model , X_test , repeat_num , feat_dims , reverse=False)
prob_te_2 = getResultByDiffSeq( revmodel , X_test , repeat_num , feat_dims , reverse=True )

ans1 = np.mean(np.array([prob_te_1 , prob_te_2]) , axis=0)

ans1 = np.argmax(ans1 , axis=1)
print(ans1.shape)
print(len(name))
"""
f_pred = open( 'LSTM_average_'+pred_filename , 'w')
w = csv.writer(f_pred)
...
trainer = Adadelta(lr = 1.0 , rho = 0.95 , epsilon = 1e-5 )
combine_model.compile(loss = 'categorical_crossentropy' , optimizer = trainer)

prob_cv_1 = getResultByDiffSeq( model , X_cv , repeat_num , feat_dims , reverse=False)
prob_cv_2 = getResultByDiffSeq( revmodel , X_cv , repeat_num , feat_dims , reverse=True )
